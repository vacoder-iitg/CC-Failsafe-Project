...
const Notifications = ({ notifications, unreadCount, markAllRead, user }) => {
    // 1. Fetch HoD messages out of the render logic so we can check their length
    ...
    // 2. Use totalItems for empty state calculation
    const totalItems = notifications.length + hodMsgs.length;
    ...
    return (
        <div style={{ padding: '28px 32px', backgroundColor: 'transparent', minHeight: '100vh' }}>
            ...
            <p style={{ color: '#6b7280', fontSize: '14px', marginBottom: '24px' }}>
                Actions taken on students and messages from HoD are logged here.
            </p>

            {/* 3. Check against totalItems instead of just notifications.length */}
            {totalItems === 0 ? (
