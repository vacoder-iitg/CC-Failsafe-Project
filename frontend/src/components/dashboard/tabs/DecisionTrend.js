...
const DecisionTrend = React.memo(({ classInsights, loadingInsights, user, sharedCache, setSharedCache }) => {
    ...
    // Use shared cache if provided, otherwise fallback to local state
    ...
    if (loadingInsights || !classInsights) {
        return (
            <div style={{ padding: '60px 20px', textAlign: 'center', backgroundColor: 'white', borderRadius: '12px', border: '1px solid #e5e7eb', marginTop: '10px' }}>
                ...
                />
                <h3 style={{ color: '#1e293b', margin: '0 0 10px 0' }}>Deep AI Decision Analysis...</h3>
                <p style={{ color: '#64748b', margin: 0, fontSize: '14px' }}>Visualizing student risk paths through the neural network.</p>
            </div>
        );
    }
